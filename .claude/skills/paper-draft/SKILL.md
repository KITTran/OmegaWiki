...
> Đánh giá Review LLM tùy chọn cho từng phần. Xuất thư mục paper/ có thể biên dịch.
...
- `--sections` *(tùy chọn)*: chỉ viết các phần được chỉ định (ví dụ: `--sections 3,4` chỉ viết Phương pháp + Thí nghiệm); dùng cho viết gia tăng
...
- Thư mục `paper/` (dưới thư mục gốc dự án wiki):
  ...
  - `paper/math_commands.tex` — định nghĩa ký hiệu toán học chung
...
- `wiki/graph/edges.jsonl` — đồ thị mối quan hệ (xây dựng chuỗi logic lập luận)
...
3. **Loại bảng**:
   ...
   - Kết quả tốt nhất in đậm, tốt thứ hai gạch chân
...
- Chèn tham chiếu `\ref{fig:name}` / `\ref{tab:name}` đến hình ảnh/bảng
- Sử dụng ký hiệu được định nghĩa trong `math_commands.tex`
- Bắt đầu mỗi đoạn với câu chủ đề
...
mcp__llm-review__chat:
  system: "Bạn là một nhà nghiên cứu ML cao cấp đang đánh giá một phần của bản thảo bài báo.
           ...
           Chỉ ra bất kỳ mẫu ngôn ngữ nghe giống AI còn lại nào.
  ...
  message: |
    ...
    5. Nội dung thiếu mà người đánh giá sẽ mong đợi?
```
...
- `mcp__llm-review__chat` — đánh giá từng phần (tùy chọn, --review) + đánh giá chéo toàn bài báo (Bước 5)
...
- `WebFetch` — lấy BibTeX DBLP / CrossRef
