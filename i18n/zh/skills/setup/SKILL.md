...
✗  SmartSync               — 未配置（通过 rsync/SSH 把改动推送到对端机器的 hook）
```
...
#### 4e：SmartSync（同步到另一台机器）

**说明**：「SmartSync 是一个 `PostToolUse` hook，在每次 `Write|Edit` 之后通过
`rsync` + SSH 把仓库内容（raw/、wiki/ 以及未被 Git 跟踪的文件）推送到另一台
对端机器。当你在 Linux/WSL 与 macOS 之间来回切换并希望两台都同步时很有用。
**默认情况下在新 clone 的仓库中处于关闭状态**。」

**询问**："是否需要配置 SmartSync 将改动推送到另一台机器？(y/n)"

如果回答**否**，跳过本节。

如果回答**是**，继续询问：

1. `username@<对端主机>` — 例如 `tuank@kiets-macbook` 或 `tuank@192.168.1.20`。
   必须是当前机器能够 SSH 登录的主机（已配置 SSH 密钥或 `~/.ssh/config` 中的别名）。
2. **对端机器上的目标路径** — 绝对路径，例如
   `/Users/tuank/Documents/Projects/xrayWiki`。若末尾缺 `/`，skill 自动补上。

**写入配置** 到 `.claude/hooks/smartsync.conf`（已在 `.gitignore` 中），基于模板
`config/smartsync.conf.example`：

```bash
cp config/smartsync.conf.example .claude/hooks/smartsync.conf
# 通过 Edit 把占位符替换成用户给出的值：
#   SMARTSYNC_REMOTE="<username@host>:<带斜杠结尾的路径>"
chmod 600 .claude/hooks/smartsync.conf
```

**注册 hook** 到 `.claude/settings.local.json`（per-machine，已在 `.gitignore`
中）—— **不要**写入 `.claude/settings.json`，那是团队共享配置。在 `PostToolUse`
下添加 matcher 为 `Write|Edit` 的条目，执行 `./.claude/hooks/SmartSync.sh`。
若 `settings.local.json` 不存在，先从 `config/settings.local.json.example`
复制；若已存在，必须**先读取**，**合并到现有 hooks**（不要替换数组），然后 `Edit`。条目结构：
...
如果 `settings.local.json` 中同一 matcher 下已存在 SmartSync 条目，不要重复
添加，仅提示用户即可。

提示：Claude Code 按 user → project → local 顺序合并 `settings.json` ←
`settings.local.json`，因此 hook 正常生效，无需改动 `settings.json`。

**结束前做一次连通性测试**：

```bash
ssh -o BatchMode=yes -o ConnectTimeout=5 <username@host> "echo ok" \
  || echo "WARN: SSH 未就绪 — 请先配置 ~/.ssh/config 或执行 ssh-copy-id 后重试。"
command -v rsync >/dev/null || echo "WARN: 当前机器需要安装 rsync。"
```

**提示用户**：SmartSync 是单向的，只把当前机器推到所配置的对端。每台机器上都需
要再次运行 `/setup` 来声明它各自的对端。
