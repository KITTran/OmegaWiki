---
description: Quét toàn bộ wiki để phát hiện các vấn đề về tình trạng và tạo báo cáo khuyến nghị sửa chữa theo cấp độ (bao gồm tất cả 9 loại thực thể + tính nhất quán của đồ thị)
---
...
> Bao gồm tất cả 9 loại thực thể, bao gồm tính hợp lý của độ tin cậy khẳng định, tính đầy đủ của lý do thất bại ý tưởng,
...
- `wiki/foundations/*.md` — các trang nền tảng
- `wiki/graph/edges.jsonl` — kiểm tra tính nhất quán của cạnh đồ thị
- `wiki/graph/citations.jsonl` — kiểm tra tính nhất quán của trích dẫn thư mục
...
3. **Các trường bắt buộc bị thiếu** (tất cả 9 loại thực thể):
...
3. **Tính hợp lệ loại cạnh**: các cạnh ngữ nghĩa sử dụng các tập loại nhận biết điểm cuối hiện tại; các loại paper-paper / paper-concept cũ tạo ra cảnh báo di chuyển
4. **Độ tin cậy cạnh**: các cạnh ngữ nghĩa paper-paper và paper-concept của `/ingest` sử dụng `confidence: high|medium|low`
5. **Lớp trích dẫn**: các hàng `graph/citations.jsonl` sử dụng `type: cites`, nguồn/ngày hợp lệ, điểm cuối paper và không có trường confidence
6. **Các nút treo**: các trang wiki được tham chiếu bởi from/to phải tồn tại
...
- **graph files không tồn tại**: bỏ qua các kiểm tra tệp đồ thị bị thiếu, ghi chú trong báo cáo
