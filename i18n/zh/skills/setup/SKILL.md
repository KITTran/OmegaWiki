...
**说明**：「SmartSync 是一个 `Stop` hook，在 Claude Code 停止时通过 `rsync` + SSH
把仓库内容（raw/、wiki/ 以及未被 Git 跟踪的文件）推送到另一台对端机器。
当你在 Linux/WSL 与 macOS 之间来回切换并希望两台都同步时很有用。
...
中）—— **不要**写入 `.claude/settings.json`，那是团队共享配置。在 `Stop` 下添加
一个执行 `./.claude/hooks/SmartSync.sh` 的条目（timeout 60 秒）。若
`settings.local.json` 不存在，先从 `config/settings.local.json.example` 复制；
若已存在，必须**先读取**，**合并到现有 `Stop` 数组**（不要替换数组），然后 `Edit`。
条目结构：
...
如果 `settings.local.json` 的 `Stop` 数组中已存在 SmartSync 条目，不要重复
添加，仅提示用户即可。
